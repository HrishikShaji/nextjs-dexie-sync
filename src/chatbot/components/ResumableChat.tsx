...
const WorkingResumableChat = () => {
	...
	// Update URL when state changes (outside of render cycle)
	useEffect(() => {
		if (currentSession) {
			updateURL(currentSession, lastChunkIndex, messages, '');
		} else if (!isStreaming && messages.length > 0) {
			// Clear URL params when session is complete
	...
	useEffect(() => {
		...
		if (sessionId) {
			...
			if (messagesData) {
					...
					if (streamingData) {
						try {
							...
							if (restoredMessages.length > 0 && restoredMessages[restoredMessages.length - 1].role === 'assistant') {
								...
								restoredMessages[restoredMessages.length - 1].content += decodedStreaming;
							} else {
								// Create new assistant message
								restoredMessages.push({ role: 'assistant', content: decodedStreaming });
							}
						} catch (error) {
				...
				try {
					const decodedStreaming = decodeURIComponent(streamingData);
					setMessages([{ role: 'assistant', content: decodedStreaming }]);
					console.log('Restored streaming-only content:', decodedStreaming);
				} catch (error) {
					console.error('Failed to parse streaming content:', error);
				}
			}
	...
	const updateURL = (sessionId, chunkIndex, currentMessages, streamingContent = '') => {
		if (!sessionId) return;
		...
		if (currentMessages && currentMessages.length > 0) {
			// For URL storage, we need to separate the complete messages from the streaming part
			let messagesToStore = [...currentMessages];
			let streamingToStore = streamingContent;

			// If we're streaming and the last message is an assistant message, 
			// we need to split it between completed and streaming parts
			if (streamingContent && messagesToStore.length > 0 &&
				messagesToStore[messagesToStore.length - 1].role === 'assistant') {
				// For simplicity, we'll store the complete current state
				// The streaming content parameter will be empty since it's already in the message
				streamingToStore = '';
			}

			params.set('messages', encodeURIComponent(JSON.stringify(messagesToStore)));
		}

		if (streamingContent) {
			params.set('streaming', encodeURIComponent(streamingContent));
		}
		...
		eventSource.onmessage = (event) => {
			try {
				...
				if (data.type === 'content') {
					...
					// No need to move content - it's already in messages
